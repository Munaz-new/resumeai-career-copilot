...
export default function BuilderPage() {
      ...
      <Layout>
        <div className="py-6">
          <div className="text-center mb-2">
            ...
            <p className="text-muted-foreground text-sm mt-1">Guided in 4 steps — then a live editor with real-time ATS feedback.</p>
          </div>
          <WizardSteps onComplete={({ profile, targetRole, level, jobDescription }) => {
  ...
  return (
    <Layout>
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{draft.targetRole || "—"} • {draft.profile} • {draft.level}</p>
          </div>
          <div className="flex items-center gap-2">
            {savedAt && <span className="hidden sm:inline-flex items-center gap-1 text-[11px] font-medium text-muted-foreground mr-1"><Check className="w-3 h-3 text-success" />Saved automatically</span>}
            <select value={draft.template} onChange={(e) => update({ template: e.target.value as TemplateId })} className="text-xs border border-border rounded-lg px-2 py-1.5 bg-background">
              {TEMPLATES.map((t) => <option key={t.id} value={t.id}>{t.label}</option>)}
            </select>
            <Button variant="outline" size="sm" onClick={() => handleExport(draft)} disabled={exporting}><Download className="w-4 h-4 mr-1" />{exporting ? "Exporting…" : "PDF"}</Button>
        ...
        <div className="grid lg:grid-cols-[260px_minmax(0,1fr)_320px] gap-6">
          <div className={cn("space-y-4", mobileView === "preview" && "hidden lg:block")}>
            ...
            <div className="dashboard-card">
              <h3 className="font-heading font-bold text-foreground text-sm mb-3 flex items-center gap-2"><FileText className="w-4 h-4 text-primary" />Contact</h3>
              <div className="space-y-2">
                ...
                <Input placeholder="LinkedIn URL" value={draft.contact.links.find((l) => l.label === "LinkedIn")?.url || ""} onChange={(e) => { const others = draft.contact.links.filter((l) => l.label !== "LinkedIn"); const url = e.target.value; update({ contact: { ...draft.contact, links: url ? [...others, { label: "LinkedIn", url }] : others } }); }} />
                <Input placeholder="GitHub URL" value={draft.contact.links.find((l) => l.label === "GitHub")?.url || ""} onChange={(e) => { const others = draft.contact.links.filter((l) => l.label !== "GitHub"); const url = e.target.value; update({ contact: { ...draft.contact, links: url ? [...others, { label: "GitHub", url }] : others } }); }} />
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className={cn(mobileView === "edit" && "hidden lg:block")}>
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground mb-2 text-center font-bold">Live Preview • {TEMPLATES.find((t) => t.id === draft.template)?.label}</div>
      ...
      <SaveAccountModal open={showSaveAccount} onOpenChange={setShowSaveAccount} title="Nice resume! Save it to your account?" description="You're in Guest Mode — create a free account to save this resume, sync across devices, and pick up where you left off." />
    </Layout>
  );
}
