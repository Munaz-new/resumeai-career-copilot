...
function ExperienceBody({ section }: { section: ResumeSection }) { const d = section.data as SectionData["experience"]; if (!d.items.length) return <p className="text-[9px] italic text-zinc-400">Add work history</p>; return <div className="space-y-5">{d.items.map((e, i) => <article key={i}><div className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 items-start"><div className="min-w-0"><p className="font-bold text-[9.5px] uppercase tracking-[0.04em] text-[#343434] break-words [overflow-wrap:anywhere]">{e.role}</p><p className="text-[8.5px] text-[#555] break-words [overflow-wrap:anywhere]">{e.company}</p></div><p className="shrink-0 text-[8px] text-[#666] text-right whitespace-nowrap">{e.start} – {e.end}</p></div><ul className="mt-2 space-y-1">{e.bullets.filter(Boolean).map((b, j) => <li key={j} className="text-[8.5px] leading-[1.45] text-[#4a4a4a] pl-2.5 -indent-2.5 break-words [overflow-wrap:anywhere]">• {b}</li>)}</ul></article>)}</div>; }
function GenericBody({ section }: { section: ResumeSection }) { const text = "text-[9px] leading-[1.45] text-[#4a4a4a] break-words [overflow-wrap:anywhere]"; switch (section.type) { case "projects": { const d = section.data as SectionData["projects"]; return d.items.length ? <div className="space-y-3">{d.items.map((p, i) => <article key={i}><div className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 items-start"><p className="font-bold text-[9.5px] break-words [overflow-wrap:anywhere]">{p.name}</p><p className="text-[8px] text-[#666] text-right break-words [overflow-wrap:anywhere]">{p.tech}</p></div>{p.link && <p className="text-[7.5px] text-[#777] break-all">{p.link}</p>}<ul className="mt-1">{p.bullets.filter(Boolean).map((b, j) => <li key={j} className={text}>• {b}</li>)}</ul></article>)}</div> : <Placeholder text="Add a project" />; } case "certifications": { const d = section.data as SectionData["certifications"]; return d.items.length ? <ul className="space-y-1.5">{d.items.map((c, i) => <li key={i} className={text}><strong>{c.name}</strong> — {c.issuer}{c.date ? ` (${c.date})` : ""}</li>)}</ul> : <Placeholder text="Add certifications" />; } default: return <Placeholder text={`Add ${sectionTitle(section.type).toLowerCase()}`} />; } }
