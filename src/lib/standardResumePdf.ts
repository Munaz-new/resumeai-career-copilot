...
function drawSectionBody(doc: jsPDF, section: ResumeSection, x: number, y: number, width: number) {
  switch (section.type) {
    ...
    case "experience":
      return drawExperience(doc, section.data as SectionData["experience"], x, y, width);
    case "projects":
      return drawProjects(doc, section.data as SectionData["projects"], x, y, width);
    case "education":
      return drawEducation(doc, section.data as SectionData["education"], x, y, width);
    case "certifications": {
...
export function exportStandardResumePdf(draft: ResumeDraft) {
  ...
  if (c.links.length) {
    y = drawWrapped(doc, c.links.map((l) => `${l.label}: ${l.url}`).join("  •  "), margin, y + 1, contentWidth, 9, MUTED, false, 1.45);
  }
