...
export interface ProjectItem { name: string; tech: string; link?: string; bullets: string[]; }
export interface ExperienceItem { role: string; company: string; start: string; end: string; bullets: string[]; }
export interface EducationItem { school: string; degree: string; start: string; end: string; details?: string; }
...
export const TARGET_ROLES = [
  "Software Developer", "Frontend Developer", "Full Stack Developer", "AI & Data Science",
  "Cybersecurity", "UI/UX Designer", "Data Analyst", "Business Analyst",
] as const;
...
export function recommendSections(profile: ProfileType): SectionType[] {
  if (profile === "experienced" || profile === "switcher") return ["summary", "experience", "projects", "skills", "certifications", "education"];
...
export function newSection<T extends SectionType>(type: T, order: number): ResumeSection<T> {
  const empty: SectionData = {
    summary: { text: "" }, skills: { items: [] }, projects: { items: [] }, experience: { items: [] },
    education: { items: [] }, certifications: { items: [] }, achievements: { items: [] }, activities: { items: [] },
  };
  return { id: `${type}-${Math.random().toString(36).slice(2, 8)}`, type, enabled: true, order, data: empty[type] as SectionData[T] };
}

export function createEmptyDraft(): ResumeDraft {
  return { profile: "fresher", targetRole: "", level: "none", template: "ats-pro", contact: { name: "", email: "", phone: "", location: "", links: [] }, sections: [], wizardComplete: false };
}

export function initSectionsFor(profile: ProfileType, role: string): ResumeSection[] {
  const sections = recommendSections(profile).map((t, i) => newSection(t, i));
  const skillsSection = sections.find((s) => s.type === "skills") as ResumeSection<"skills"> | undefined;
  if (skillsSection && ROLE_SKILL_PRESETS[role]) skillsSection.data.items = [...ROLE_SKILL_PRESETS[role]];
  return sections;
}
...
export function draftToPlainText(d: ResumeDraft): string {
  ...
  for (const s of sorted) { out.push(sectionTitle(s.type).toUpperCase()); out.push(sectionBody(s)); out.push(""); }
  return out.join("\n");
}

export function sectionTitle(t: SectionType): string {
  return { summary: "Professional Summary", skills: "Skills", projects: "Projects", experience: "Experience", education: "Education", certifications: "Certifications", achievements: "Achievements", activities: "Activities" }[t];
}

function sectionBody(s: ResumeSection): string {
  switch (s.type) {
    case "summary": return (s.data as SectionData["summary"]).text;
    case "skills": return (s.data as SectionData["skills"]).items.join(", ");
    case "projects": return (s.data as SectionData["projects"]).items.map((p) => `${p.name} — ${p.tech}\n${p.bullets.map((b) => "• " + b).join("\n")}`).join("\n\n");
    case "experience": return (s.data as SectionData["experience"]).items.map((e) => `${e.role} | ${e.company} (${e.start} - ${e.end})\n${e.bullets.map((b) => "• " + b).join("\n")}`).join("\n\n");
    case "education": return (s.data as SectionData["education"]).items.map((e) => `${e.degree}, ${e.school} (${e.start} - ${e.end})${e.details ? "\n" + e.details : ""}`).join("\n");
    case "certifications": return (s.data as SectionData["certifications"]).items.map((c) => `${c.name} — ${c.issuer} (${c.date})`).join("\n");
    case "achievements":
    case "activities": return (s.data as { items: string[] }).items.map((i) => "• " + i).join("\n");
  }
}
